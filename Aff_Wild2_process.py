...
for _, _, files in os.walk(annotation_train_path):
    ...
    for file_name in tqdm(sort_files):
...
annotation_train.to_csv(os.path.join(data_dir, 'train_annotation.txt'), index=False)  # 结果csv存放路径
...
annotation_eval.to_csv(os.path.join(data_dir, 'eval_annotation.txt'), index=False)  # 结果csv存放路径
